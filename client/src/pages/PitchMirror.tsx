      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: MUTED, margin: "2px 0 0", opacity: 0.7 }}>
        ...
        {view === "RESULTS" && result && (
          <div style={{ marginTop: 32 }}>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 8, flexWrap: "wrap" }}>
              ...
              <LinkedInShareButton founderStage={result.founderStage} />
            </div>
...
function LinkedInShareButton({ founderStage }: { founderStage?: string }) {
  const stageLabel = founderStage === "idea" ? "idea stage"
    : founderStage === "building" ? "building stage"
    : founderStage === "early_revenue" ? "early revenue stage"
    : founderStage === "scaling" ? "scaling stage"
    : "startup";

  function handleLinkedIn() {
    const text = `Just got AI-powered investor feedback on my ${stageLabel} pitch via AgenThinkMesh PitchMirror — free, no login needed. Try it: ${window.location.origin}/pitchmirror`;
    const url = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.origin + "/pitchmirror")}&summary=${encodeURIComponent(text)}`;
    window.open(url, "_blank", "noopener,noreferrer,width=600,height=600");
    trackEvent("pitchmirror_linkedin_share", { founderStage });
  }
