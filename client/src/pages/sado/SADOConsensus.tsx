  ...
  return (
    ...
    <div className="min-h-screen bg-[oklch(0.10_0.02_255)]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
        ...
        {/* ── B.5 CTA footer ────────────────────────────────────────────────── */}
        <section className="rounded-2xl border border-blue-500/20 bg-gradient-to-br from-blue-950/40 via-slate-900/60 to-slate-900/40 p-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div className="flex-1">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-blue-400 mb-2">
                Architecture Preview — Consensus Governance Engine
              </p>
              <h2 className="text-xl font-bold text-white mb-2 leading-snug">
                Want to see the Council evaluate your own data estate?
              </h2>
              <p className="text-sm text-slate-400 leading-relaxed max-w-xl">
                Request a live walkthrough using your organisation's own residency, privacy, and audit policies.
                Our team will configure the Council of Ten against your jurisdictions and run a real decision scenario
                — at no charge for the first session.
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0">
              <Link href={`/contact${buildProspectQuery(prospect)}`}>
                <button className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold transition-colors shadow-lg shadow-blue-900/30">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg border border-slate-600 bg-slate-800/60 hover:border-slate-500 hover:bg-slate-700/60 text-slate-300 hover:text-white text-sm font-semibold transition-colors"
              >
                {copyState === "copied" ? (
                  <><CheckCircle2 className="w-4 h-4 text-emerald-400" /><span className="text-emerald-400">Link copied</span></>
                ) : (
                  <><Copy className="w-4 h-4" />Copy prospect link</>
                )}
              </button>
            </div>
          </div>
