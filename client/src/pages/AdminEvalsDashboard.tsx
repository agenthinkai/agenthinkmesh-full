 ...
 *
 * Export:
 *   Export CSV button in the header — three modes: raw rows, by-day, by-provider.
 *   Uses the adminEvalStats.exportCsv mutation (server-side CSV generation).
...
type ExportType = "raw" | "byDay" | "byProvider";

const EXPORT_OPTIONS: { label: string; value: ExportType; desc: string }[] = [
  { label: "Raw Rows",       value: "raw",        desc: "Individual log entries (up to 5,000)" },
  { label: "By Day",         value: "byDay",       desc: "Daily aggregation" },
  { label: "By Provider",    value: "byProvider",  desc: "Provider / model breakdown" },
];
...
function ExportCsvButton({ days }: { days: number }) {
  ...
  const exportMutation = trpc.adminEvalStats.exportCsv.useMutation({
    onSuccess: (data) => {
      if (data.csv) {
        ...
        setStatus("error");
        setErrorMsg("No data returned for this window.");
  ...
  function handleExport(exportType: ExportType) {
    ...
    setStatus("loading");
    setErrorMsg("");
    exportMutation.mutate({ days, exportType, limit: 5_000 });
  }
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 w-56 bg-[#0f1117] border border-white/15 rounded-lg shadow-xl z-50 overflow-hidden">
          <div className="px-3 py-2 border-b border-white/10">
            <p className="text-[11px] text-slate-400 uppercase tracking-wider font-medium">
              Export window: last {days}d
            </p>
          </div>
          {EXPORT_OPTIONS.map((opt) => (
            <button
              ...
              className="w-full text-left px-3 py-2.5 hover:bg-white/8 transition-colors group"
            >
              <div className="text-[13px] text-white font-medium group-hover:text-amber-300 transition-colors">
                {opt.label}
              </div>
    ...
    <div className="flex min-h-screen bg-[#080a0e] text-white">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
          ...
          {/* Controls: window selector + export */}
