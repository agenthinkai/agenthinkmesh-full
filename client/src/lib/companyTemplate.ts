...
export interface TelcoDefaults {
  revenue: number;          // KD M FY2025
  ebitda: number;           // KD M FY2025
  ebitdaMargin: number;     // % FY2025
  netIncome: number;        // KD M FY2025
  marketCap: number;        // KD M
  enterpriseValue: number;  // KD M
  evEbitda: number;         // current multiple
  peerEvEbitda: number;     // benchmark peer multiple (e&)
  digitalRevenuePct: number; // % of total revenue
  capexRevenuePct: number;  // % capex / revenue
  subscribers: number;      // M
  roe: number;              // %
}

export interface EngineeringDefaults {
  revenue: number;          // $M FY2025
  ebitda: number;           // $M FY2025
  ebitdaMargin: number;     // %
  employees: number;
  digitalRevenuePct: number; // % of revenue from digital/advisory
  fixedFeePct: number;      // % of revenue from fixed-fee contracts
  outcomePct: number;       // % of revenue from outcome-based contracts
  utilizationRate: number;  // %
  revenuePerHead: number;   // $K
}
...
export interface SliderOverrides {
  // Lever 1: AI Adoption
  aiName?: string;            // default: "AI Adoption"
  aiLow?: string;             // default: "None"
  aiHigh?: string;            // default: "Fully adopted"
  aiFmt?: (v: number) => string;
  // Lever 2: Pricing Power
  ...
  // Lever 3: Market Share
  shareName?: string;         // default: "Market Share"
  shareLow?: string;          // default: "1%"
  shareHigh?: string;         // default: "20%"
  shareFmt?: (v: number) => string;
  // Lever 4: Revenue Growth
  growthName?: string;        // default: "Revenue Growth"
  growthLow?: string;         // default: "−5%"
  growthHigh?: string;        // default: "15%"
  growthFmt?: (v: number) => string;
  // Lever 5: Cost Inflation
  costInflName?: string;      // default: "Cost Inflation"
  costInflLow?: string;       // default: "0%"
  costInflHigh?: string;      // default: "15%"
  costInflFmt?: (v: number) => string;
  // Lever 6: Digital Advisory Growth
  daGrowthName?: string;      // default: "Digital Advisory Growth"
  daGrowthLow?: string;       // default: "Current"
  daGrowthHigh?: string;      // default: "Scaled"
  daGrowthFmt?: (v: number) => string;
  // Lever 7: GCC Expansion
...
export interface DataSourcesConfig {
  sources: string[];       // list of named data sources
  lastUpdated: string;     // e.g. "25 June 2026"
  disclaimer: string;      // credibility statement
}
...
export interface CompanyTemplate {
  ...
  // Telco mode — EV/EBITDA rerating engine for Zain and similar
  telcoMode?: boolean;
  telcoDefaults?: TelcoDefaults;
  // Engineering mode — AI value capture engine for KEO and similar
  engineeringMode?: boolean;
  engineeringDefaults?: EngineeringDefaults;
  currency?: string;           // e.g. "KD", "$", "AED" — defaults to "$"
...
export const KEO_TEMPLATE: CompanyTemplate = {
  ...
  dataSources: {
    sources: [
      ...
      "Public Financial Estimates",
      "Industry Publications",
    ],
    lastUpdated: "25 June 2026",
    disclaimer: "Built using publicly available information. No internal company data, ERP systems, or confidential disclosures have been used.",
  },
  engineeringMode: true,
  engineeringDefaults: {
    // Source: KEO public estimates, GCC engineering sector benchmarks (2025)
    // KEO is privately held; figures are model estimates from industry benchmarks
    revenue: 210,           // $M estimated FY2025
    ebitda: 30,             // $M estimated (14.3% margin)
    ebitdaMargin: 14.3,     // % estimated
    employees: 2400,
    digitalRevenuePct: 6,   // % of revenue from digital/advisory services
    fixedFeePct: 12,        // % of revenue from fixed-fee contracts
    outcomePct: 2,          // % of revenue from outcome-based contracts
    utilizationRate: 68,    // %
    revenuePerHead: 87.5,   // $K per employee
  },
  geoLabels: { ksa: "Saudi Arabia", uae: "UAE", kw: "Kuwait", other: "Other GCC" },
  sliderOverrides: {
    // KEO-specific executive levers — replace generic labels with board-level decisions
    aiName: "AI Productivity Capture",
    aiLow: "Manual Delivery",
    aiHigh: "AI-Augmented",
    aiFmt: (v: number) => v < 30 ? "Manual" : v < 60 ? "Partial AI" : "AI-Augmented",
    pricingName: "Contract Model Shift",
    pricingLow: "Time & Materials",
    pricingHigh: "Outcome-Based",
    pricingFmt: (v: number) => v < 25 ? "T&M" : v < 60 ? "Mixed" : "Outcome",
    shareName: "Fixed-Fee Contract %",
    shareLow: "Low",
    shareHigh: "High",
    shareFmt: (v: number) => `${v}% share`,
    growthName: "Revenue Growth",
    growthLow: "Declining",
    growthHigh: "Strong Growth",
    growthFmt: (v: number) => `${v > 0 ? "+" : ""}${v}%`,
    costInflName: "Operating Cost Pressure",
    costInflLow: "Controlled",
    costInflHigh: "Elevated",
    costInflFmt: (v: number) => `${v}%`,
    daGrowthName: "Digital Advisory Revenue",
    daGrowthLow: "Niche",
    daGrowthHigh: "Core Business",
    daGrowthFmt: (v: number) => v < 25 ? "Niche" : v < 55 ? "Growing" : v < 80 ? "Material" : "Core",
    gccName: "International Expansion",
    gccLow: "GCC Only",
    gccHigh: "Global Reach",
    gccFmt: (v: number) => v < 25 ? "GCC" : v < 60 ? "MENA" : "Global",
  },
  defaults: {
  ...
  scenarios: {
    ...
    ainative: {
      name: "KEO Becomes AI-Native", tag: "transform before a rival does",
      set: { ai: 92, pricing: 90, share: 11, growth: 9, costInfl: 3, daGrowth: 80, gcc: 25 },
      // This scenario models KEO *becoming* the AI-native firm — the aspirational transformation.
      // The high ai/pricing/daGrowth values represent KEO's own transformation, not an external threat.
      // EV is higher because this is the highest-upside scenario.
      // The Margin Compression scenario is the true downside threat case.
      rec: "Decide to become the AI-native engineering firm before a rival does. The economics that threaten KEO are the same ones it can own — but only by changing productivity and pricing simultaneously. This is the highest-EV scenario because it captures the software-adjacent multiple.",
      conf: "High",
...
export const WARBA_TEMPLATE: CompanyTemplate = {
  ...
  dataSources: {
    sources: [
      "KPMG Kuwait Listed Banks YE2025 (March 2026)",
      "Warba Bank Annual Report FY2025",
      "Public Investor Presentations",
      "Boursa Kuwait Market Data",
      "CBK Public Regulatory Disclosures",
      "GCC Islamic Banking Benchmarks",
    ],
    lastUpdated: "25 June 2026",
    disclaimer: "Built using publicly available information. No internal company data, ERP systems, or confidential disclosures have been used.",
  },
  ...
  geoLabels: { ksa: "Saudi Arabia", uae: "UAE", kw: "Kuwait", other: "Other GCC" },
  sliderOverrides: {
    // Warba-specific executive levers — replace generic labels with board-level decisions
    aiName: "Digital Banking Adoption",
    aiLow: "Branch-Led",
    aiHigh: "Digital-First",
    aiFmt: (v: number) => v < 30 ? "Branch-Led" : v < 60 ? "Hybrid" : "Digital-First",
    pricingName: "Cost Efficiency Programme",
    pricingLow: "No Change",
    pricingHigh: "Full Transformation",
    pricingFmt: (v: number) => v < 25 ? "No Change" : v < 60 ? "Underway" : "Transformed",
    shareName: "Deposit Growth",
    shareLow: "Flat",
    shareHigh: "Aggressive",
    shareFmt: (v: number) => `${v}% share`,
    growthName: "Net Income Growth",
    growthLow: "Declining",
    growthHigh: "Strong Growth",
    growthFmt: (v: number) => `${v > 0 ? "+" : ""}${v}%`,
    costInflName: "Operating Cost Pressure",
    ...
    costInflHigh: "Elevated",
    costInflFmt: (v: number) => `${v}%`,
    daGrowthName: "Gulf Bank Integration",
    daGrowthLow: "No Merger",
    daGrowthHigh: "Full Integration",
    daGrowthFmt: (v: number) => v < 25 ? "No Merger" : v < 55 ? "Stake Only" : v < 80 ? "Merger Announced" : "Integrated",
    gccName: "Mortgage Market Expansion",
    gccLow: "Law Delayed",
    gccHigh: "Law Active + Scale",
    gccFmt: (v: number) => v < 25 ? "Delayed" : v < 55 ? "Pending" : v < 80 ? "Active" : "Scaled",
  },
  defaults: {
...
export const ZAIN_TEMPLATE: CompanyTemplate = {
  ...
  dataSources: {
    sources: [
      "Zain Group FY2025 Annual Report",
      "Zain FY2025 Investor Presentation (Feb 2026)",
      "Zain Q1 2026 Earnings Release (May 2026)",
      "Boursa Kuwait Market Data (ZAIN.KW)",
      "Public Telecom Industry Benchmarks",
    ],
    lastUpdated: "25 June 2026",
    disclaimer: "Built using publicly available information. No internal company data, ERP systems, or confidential disclosures have been used.",
  },
  telcoMode: true,
  telcoDefaults: {
    // Source: Zain FY2025 Press Release (Feb 2026), Yahoo Finance ZAIN.KW (Jun 2026)
    revenue: 2290,          // KD M FY2025 (+14% YoY)
    ebitda: 780,            // KD M FY2025 (EBITDA margin 34%)
    ebitdaMargin: 34.1,     // % FY2025
    netIncome: 239,         // KD M FY2025 (+103% YoY, 13-year high)
    marketCap: 2620,        // KD M (Yahoo Finance Jun 2026)
    enterpriseValue: 4280,  // KD M (market cap + net debt ~KD 1.66B)
    evEbitda: 5.09,         // current multiple (EV 4,280 / EBITDA 780 = 5.49x, rounded)
    peerEvEbitda: 9.0,      // e& benchmark (8–10x, midpoint)
    digitalRevenuePct: 37,  // % data revenue of total (USD 2.8B / USD 7.5B)
    capexRevenuePct: 20,    // % capex / revenue (USD 1.5B / USD 7.5B)
    subscribers: 50.9,      // M across 8 markets
    roe: 15.66,             // % FY2025
  },
  sliderOverrides: {
    // Zain-specific executive levers — replace generic labels with board-level decisions
    aiName: "AI & Network Intelligence",
    aiLow: "Manual Operations",
    aiHigh: "AI-Driven Network",
    aiFmt: (v: number) => v < 30 ? "Manual" : v < 60 ? "Partial AI" : "AI-Driven",
    ...
    shareName: "Enterprise Services Growth",
    shareLow: "Organic Only",
    shareHigh: "Aggressive",
    shareFmt: (v: number) => `${v}% share`,
    growthName: "ZainTECH Revenue Growth",
    growthLow: "Flat",
    growthHigh: "Hyper-Growth",
    growthFmt: (v: number) => `${v > 0 ? "+" : ""}${v}%`,
    costInflName: "Infrastructure Investment",
    costInflLow: "Minimal Capex",
    costInflHigh: "Heavy Investment",
    costInflFmt: (v: number) => `${v}% of rev`,
    daGrowthName: "Africa Fintech Scale",
    daGrowthLow: "Today",
    daGrowthHigh: "Full Scale",
    daGrowthFmt: (v: number) => v < 25 ? "Early" : v < 55 ? "Growing" : v < 80 ? "Material" : "Scaled",
    gccName: "New Verticals Acceleration",
    gccLow: "Organic Only",
    gccHigh: "Aggressive Build/Buy",
    gccFmt: (v: number) => v < 25 ? "Organic" : v < 60 ? "Selective" : "Aggressive",
  },
  ...
  scenarios: {
    ...
    ainative: {
      name: "Hyperscaler Threat", tag: "the existential case — partnership or displacement",
      set: { ai: 88, pricing: 85, share: 9, growth: 5, costInfl: 6, daGrowth: 55, gcc: 35 },
...
export function getTemplate(id: string): CompanyTemplate {
  return TEMPLATES[id] ?? TEMPLATES[id.toLowerCase()] ?? KEO_TEMPLATE;
}
