...
## Three-Task Sprint (e22ccdb9 base)

### Task 1 — Outcome Completion Rate in Pipeline Summary
- [x] Client: compute outcomeCount (decisionOutcome not null) and totalCount from allRows in HistoryTab
- [x] Client: add "[n] of [total] deals have outcomes recorded" stat line to Pipeline Summary widget
- [x] Client: hidden when totalCount is 0
- [x] Client: derived from data already in memory — no new queries
- [x] No schema changes

### Task 2 — 90-Day Recency Filter on Pattern Signal Queries
- [x] Server: in patternInsight procedure, filter getOutcomeHistory results to createdAt >= 90 days ago before pattern frequency calculation
- [x] Server: in agentCalibration procedure, apply same 90-day filter to outcome records
- [x] Server: MIN_GROUP threshold (3 records) applies after the 90-day filter
- [x] Server: if fewer than 3 records within 90 days → return { type: "none" } (same as current insufficient data path)
- [x] Server: no changes to client — output shape unchanged
- [x] No schema changes

### Task 3 — Export Stale Deals as CSV
- [x] Client: add "Export stale deals" button in History tab, visible only when staleDeals.length > 0
- [x] Client: on click, generate CSV client-side from staleDeals array
- [x] Client: CSV columns: Deal Name, Stage, Days in Stage, Triage Score, Classification, Created At
- [x] Client: filename: stale-deals-YYYY-MM-DD.csv (today's date)
- [x] Client: trigger browser download via Blob URL
- [x] Client: no new API endpoint, uses data already in memory
- [x] No schema changes

### Shared Constraints
- [x] tsc --noEmit EXIT:0 after each task
- [x] Tests: 693/693 passing throughout
- [x] No regressions to History list, nudge rows, pattern signal, Boardroom view, PDF badge, triage result screen
