...
export const dealScreenings = mysqlTable("deal_screenings", {
  ...
  councilMode: mysqlEnum("councilMode", ["gcc", "global_vc", "india_pe", "gcc_equities", "infrastructure"]).default("global_vc").notNull(),
...
export const orchestrationUnits = mysqlTable("orchestration_units", {
  ...
  outputTokens: int("output_tokens").notNull().default(0),
  latencyMs: int("latency_ms"),
  retryNumber: int("retry_number"),
  tiersUsed: text("tiers_used"),  // JSON array e.g. ["SMALL","MID"]
  finalTier: varchar("final_tier", { length: 10 }),
  attemptsCount: int("attempts_count").notNull().default(1),
  ...
  priceUsd: decimal("price_usd", { precision: 14, scale: 8 }).notNull().default("0.00000000"),
